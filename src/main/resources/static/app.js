...
async function submitPremiumOrder() {
  ...
  if (state.selectedPremMonths === 1) {
    ...
    const adminUrl = "https://t.me/stalkerbek?text=" + encodeURIComponent("Salom! Men 1 oylik Telegram Premium sotib olmoqchiman. Qabul qiluvchi: " + cleanTarget);
    if (window.Telegram && window.Telegram.WebApp && window.Telegram.WebApp.openTelegramLink) {
      window.Telegram.WebApp.openTelegramLink(adminUrl);
    } else {
      window.open(adminUrl, '_blank');
    }
    showToast("1 oylik Premium uchun adminga (@stalkerbek) yo'naltirilmoqda... 💬");
    return;
  }
...
async function fetchReferralData() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (pctEl) pctEl.innerText = "+100 / +200";
    }
  } catch (e) {}
}
